...
{if isset($smarty.get.email) }
	...
	{/if}
	<form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post" class="box" style="text-align: center">
	    <H1>Gracias por registrarse en venegangas!</H1>
	    <h3>Haga clic en <h3>
	    <input type="text" name="email" value="{$smarty.get.email}" style="display:none">
			<button type="submit" id="SubmitActive" name="SubmitActive" class="button btn btn-default button-medium">
				<span>
					<i class="icon-unlock left"></i>
					Continuar
				</span>
			</button>
  </form>
{else}
	{if !isset($email_create)}
		...
		{/if}
		<div class="row">
			<div class="col-xs-12 col-sm-13 col-lg-4">
	...
	{else}
		...
		{/if}-->
		{if isset($success)}
			<p class="alert alert-success" style="font-size:1.7em">
				{$success}
 			</p>
				<form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post" id="login_form" class="box">
					<h3 class="page-subheading">{l s='Already registered?'}</h3>
		...
		{else}
			<form action="{$link->getPageLink('authentication', true)|escape:'html':'UTF-8'}" method="post" id="account-creation_form" class="std box">
				...
				<!-- CAMPOO AÑADIDO 26/08/16 CARLOS ANSELMI PARA VALIDAR LA CONTRASEÑA! -->
					<div class="required password form-group">
						<label for="passwd">{l s='Confirm password'}  <sup>*</sup></label>
						<input type="password" class="is_required validate form-control" data-validate="isPasswd" name="confirm_passwd" id="confirm_passwd" />
						<span class="form_info" id="error_passwd" style="color:red; display: none">La clave no coinciden</span>
					</div>
					<style type="text/css">
						.incorrecto{
					...
					<script type="text/javascript">
						$(document).ready(function(){
							$('#confirm_passwd').on('focusout keyup',function(){
								if ( $(this).val()==$('#passwd').val() ){
									$(this).addClass('form-ok').removeClass('incorrecto');
									$('#error_passwd').hide();
								}else{	
									$(this).addClass('incorrecto').removeClass('form-ok');
									$('#error_passwd').show();
								}
							});
						});
