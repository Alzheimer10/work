...
class AuthControllerCore extends FrontController
{
	...
	public function postProcess()
	{
		...
		if (Tools::isSubmit('SubmitActive'))
			$this->processActive();
	}
	...
	protected function processActive()
	{
    	...
    	$sql = 'UPDATE '._DB_PREFIX_.'customer SET active=1 WHERE email="'.Tools::getValue('email').'"';
        if (Db::getInstance()->execute($sql)){
            Tools::redirect('index.php?controller='.(($this->authRedirection !== false) ? urlencode($this->authRedirection) : 'my-account'));
        }else{
            $this->errors[] = Tools::displayError('success','El usuario ya esta activado!');
        }
	}
				...
				if (!count($this->errors))
				{
					if ($customer->add())
					{
						...
						// else : redirection to the account
							...
							Tools::redirect('index.php?controller='.(($this->authRedirection !== false) ? urlencode($this->authRedirection) : 'my-account'));*/
						unset($_POST['email_create']);
						$this->context->smarty->assign('success','Se ha enviado un mensaje a: '.$customer->email.' Por favor, verifique su correo electrónico');
					}
					else
	...
	protected function sendConfirmationMail(Customer $customer)
	{
		...
		return Mail::Send(
			...
			array(
				...
				'{active_url}' => 'http://venegangas.com/venegangas/autenticacion?email='.$customer->email,
